...
base_path = r"I:\Assignment 2\temperatures"
...
# -------- PLOTTING (colors only changed) --------

# Plot 1: Seasonal Average Temperatures (Bar Plot) with viridis gradient
n1 = len(seasonal_avg)
den1 = max(n1 - 1, 1)
colors_season = [plt.cm.viridis(i / den1) for i in range(n1)]
...
# Plot 2: Top 5 Stations by Temperature Range (Bar Plot) with plasma gradient
top_range_stations = station_stats.nlargest(5, 'range')
n2 = len(top_range_stations)
den2 = max(n2 - 1, 1)
colors_range = [plt.cm.plasma(i / den2) for i in range(n2)]

plt.figure(figsize=(10, 6))
plt.bar(top_range_stations.index, top_range_stations['range'], color=colors_range)
...
# Plot 3: Temperature Stability (Std Dev) for Most Stable vs Most Variable with two-tone cividis
stability_data = station_stats.loc[stable_stations + variable_stations, 'std']
stability_labels = stable_stations + variable_stations
stable_color = plt.cm.cividis(0.25)
variable_color = plt.cm.cividis(0.85)
stability_colors = [stable_color] * len(stable_stations) + [variable_color] * len(variable_stations)
